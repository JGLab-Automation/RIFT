...
class ResourceDiscovery_OpenStack(unittest.TestCase):
    ...
    def test05_Discover_Resource(self):
        ...
        fw.vim_resource_discover(const.lp_addr, const.header, self.proj_name[0], const.cloud_acct_name)
        status = fw.vim_discover_status(const.lp_addr, const.header, self.proj_name[0], const.cloud_acct_name)
    ...
    def test06_Cloud_Account_Delete(self):
        ...
        acct = fw.get_cloud_acct_details(const.lp_addr, const.header, self.proj_name[0], const.cloud_acct_name)
        if acct:
            fw.cloud_acct_delete(const.lp_addr, const.header, self.proj_name[0], const.cloud_acct_name)
            acct = fw.get_cloud_acct_details(const.lp_addr, const.header, self.proj_name[0], const.cloud_acct_name)
            if not acct:
    ...
    def test07_Project_Delete(self):
        ...
        proj_details = fw.get_proj_details(const.lp_addr, const.header, self.proj_name[0])
        if proj_details:
            fw.proj_delete(const.lp_addr, const.header, self.proj_name[0])
            proj_details = fw.get_proj_details(const.lp_addr, const.header, self.proj_name[0])
