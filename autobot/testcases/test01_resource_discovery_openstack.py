...
class ResourceDiscoveryOpenStack(unittest.TestCase):
    ...
    def test05_Discover_Resource(self):
        ...
        fw.vim_resource_discover(const.lp_addr_default[0], const.header_default, self.proj_name[0], const.cloud_acct_name[0])
        status = fw.vim_discover_status(const.lp_addr_default[0], const.header_default, self.proj_name[0], const.cloud_acct_name[0])
        if status == "discovered":
            util.log_info(f"Resource discovery on account-name- {const.cloud_acct_name[0]} is successful. Test-case Pass.")
        else:
